...
class CNN(Model):
    def __init__(self, para_dict, embedding_fn, *args, **kwargs):
        ...
        if 'conv1_filter_size' not in para_dict:
            self.para_dict['conv1_filter_size'] = (self.para_dict['embedding_dim'], 2)  # from paper
        if 'conv2_filter_size' not in para_dict:
            self.para_dict['conv2_filter_size'] = (1, 2)
        ...
        if 'padding' not in para_dict:
            self.para_dict['padding'] = 0
    ...
    def net_init(self):
        ...
        self.conv1 = nn.Conv2d(in_channels=1,  # todo: support gapped?
                               ...
                               padding=self.para_dict['padding'])  # todo fix strides
        ...
        self.pool2 = nn.MaxPool2d(kernel_size=self.para_dict['max_pool_filter_size'],
                                  stride=self.para_dict['stride'])  # todo: make 2 a param
        width = get_width_after_conv(width, self.para_dict['max_pool_filter_size'][1], self.para_dict['stride'])
        ...
        self.logits = nn.Linear(in_features=self.para_dict['fc_hidden_dim'],
                                out_features=self.num_classes)  # todo: parameterize
    ...
    def fit(self, data_loader, test_loader=None):
        ...
        train_output_init, train_labels_init, train_loss_init = self.predict(data_loader)
        print('Initial Evaluation...')
        print('Epoch: %d: Train Loss=%.3f' % (saved_epoch, train_loss_init))  # todo: refactor total_loss to train_loss
        train_cm, train_acc, train_mcc = self.evaluate(train_output_init, train_labels_init)
        if test_loader is not None:
            print()
            test_output_init, test_labels_init, test_loss_init = self.predict(test_loader)
            print('Epoch: %d: Test Loss=%.3f' % (saved_epoch, test_loss_init))  # todo: refactor total_loss to train_loss
            test_cm, test_acc, test_mcc = self.evaluate(test_output_init, test_labels_init)
        ...
        out_metrics = {}
        for e in range(saved_epoch+1, self.para_dict['epoch']+1):
            total_loss = 0
            outputs_train = []
            labels_train = []
            ...
            if e % 10 == 0:
                self.save_model('Epoch_' + str(e), self.state_dict())
                tail_n = 5
                ...
                outputs_train, labels_train, train_loss = self.predict(data_loader)
                print('Epoch: %d: Train Loss=%.3f' % (e, train_loss))  # todo: refactor total_loss to train_loss
                train_cm, train_acc, train_mcc = self.evaluate(outputs_train, labels_train)
                ...
                if test_loader is not None:
                    # todo: take best model based on test mcc
                    #                     pdb.set_trace()
                    print('Epoch: %d: Test Loss=%.2f' % (e, test_loss))
                    ...
                    if test_mcc > best_test_mcc:
                        best_test_mcc = test_mcc
                        best_epoch = e
                        self.save_model(f'best_epoch_{e}', self.state_dict())
                        self.save_model(self.best_model_fn, self.state_dict())
                        print()
                        print(f'best_epoch={e};', f'best_test_mcc={best_test_mcc};')
                    print(20 * '=' + '\n\n')
