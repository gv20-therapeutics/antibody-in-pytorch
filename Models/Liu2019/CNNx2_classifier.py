...
class CNNx2_classifier(Model):
    ...
    def net_init(self):
        ...
        self.conv2 = nn.Conv1d(in_channels = 21, 
                               out_channels = self.para_dict['n_filter2'],
                               kernel_size = self.para_dict['filter_size'],
                               stride = 1, padding = 0)
        self.pool2 = nn.MaxPool1d(kernel_size = 2, stride = 1)
        self.fc1 = nn.Linear(in_features = (self.para_dict['seq_len']-self.para_dict['filter_size']) * self.para_dict['n_filter2'], out_features = self.para_dict['fc_hidden_dim'])
